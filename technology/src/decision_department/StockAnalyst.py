...
class StockAnalyst(object):
    ...
    def startAnalysisKLineForm(self):
        ...
        for id in self.__stockMap.keys():
            ...
            # 规则1
            rule_one_result = MADecision().ruleOne(df)
            # 规则2
            rule_two_result = MADecision().ruleTwo(df)
            # 规则3
            rule_three_result = MADecision().ruleThree(df)
            # 规则4
            rule_four_result = MADecision().ruleFour(df)
            # 规则五
            rule_five_result = MADecision().ruleFive(df)
            list2Df = [[
                id,
                self.__stockMap[id],
